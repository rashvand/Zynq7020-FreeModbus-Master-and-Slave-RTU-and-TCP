...
extern uint64_t XTime_GetInMiliSec(void);
...
eMBMasterReqErrCode eMBMasterWaitRequestFinish(LONG timeout) {
    ...
    //get now tick
    uint64_t tickstart = XTime_GetInMiliSec();
    eMBMasterEventType event;
    while(1){
      if(xEventInQueue){
        if( (eQueuedEvent&EV_MASTER_PROCESS_SUCESS)||\
        	(eQueuedEvent&EV_MASTER_ERROR_RESPOND_TIMEOUT)||\
			(eQueuedEvent&EV_MASTER_ERROR_RECEIVE_DATA)||\
			(eQueuedEvent&EV_MASTER_ERROR_EXECUTE_FUNCTION))
        {
          //get the event
      ...
      if((timeout == 0U)||((XTime_GetInMiliSec() - tickstart ) > timeout)){
